  ...
  try {
    ...
    if (!book.textContent || book.textContent.trim().length < 50) {
      return res.status(400).json({
        error:
          "Book content is too short or empty. Please upload a book with sufficient text content.",
      });
    }
    ...
    if (
      ...
      return res.status(500).json({
        error:
          "AI service returned an unexpected response. Please try generating the quiz again.",
      });
    } else {
      return res.status(500).json({
...
router.get("/book/:bookId", async (req, res) => {
  try {
    const quizzes = await Quiz.find({ bookId: req.params.bookId }).sort({
      createdAt: -1,
    });
    res.json(quizzes);
  } catch (error) {
