...
const VideoRecommendations = ({ topic = "physics" }) => {
  ...
  const fetchVideos = async () => {
    try {
      ...
      const response = await axios.get(`/api/youtube/search?topic=${encodeURIComponent(topic)}`);
    ...
    return (
      <div className="bg-white shadow-lg rounded-lg p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-xl font-semibold text-gray-800">Recommended Videos</h2>
        </div>
    ...
    <div className="bg-white shadow-lg rounded-lg p-6">
      ...
      <div className="mb-6">
        <p className="text-gray-600 mb-2">
          Related to: <span className="inline-block bg-blue-100 text-blue-800 text-sm px-3 py-1 rounded-full">
            {topic || "Physics"}
          </span>
        </p>
        {error && (
          ...
          >
            <div className="relative">
              <img
                ...
                onError={(e) => {
                  e.target.src = 'https://via.placeholder.com/300x200/4F46E5/FFFFFF?text=Video+Thumbnail';
                }}
              />
